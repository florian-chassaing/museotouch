...
from museolib.widgets.customdropdown import CustomDropDown
...
def build(app):
    ...
    #Dropdown menu
    app.dropdown = CustomDropDown(size=(200,50),pos_hint={'bottom': 1, 'center_x': .5},
            size_hint=(None, None),
            do_translate=False, do_rotate=False, do_scale=False)
    app.dropdown.menu.bind(on_select=lambda instance, x: setattr(app.dropdown.ids.mainbutton, 'text', x))
    root.add_widget(app.dropdown)
    # -------------------------------------------------------------------------
    ...
    app.imagemap = imagemap = ImageMap(
            ...
            sources=sources,
            suffix='_active')
    scat_map = Scatter(size=app.imagemap.size,
            auto_bring_to_front=False,
            pos_hint={'top': 1, 'center_x': .5},
            size_hint=(None, None), rotation=-180,
            do_translate=False, do_rotate=False, do_scale=False)
    scat_map.add_widget(app.imagemap)
    root.add_widget(scat_map)
    ...
    ordering_origin = Button(
            ...
            **kwargs)
    ordering_origin.pos_hint={'top':1,'center_x':0.3}
